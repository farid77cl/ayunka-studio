...
(function(){
  ...
  function syncSave(){ try{ const fb=JSON.parse(document.getElementById('sync-cfg').value); const ws=document.getElementById('sync-ws').value.trim()||'ayunka'; const em=document.getElementById('sync-email').value.trim(); const pw=document.getElementById('sync-pass').value; if(!em||!pw){toast('Ingresa tu email y una contraseña para la sincronización');return;} window.Sync.setCfg(fb,ws,em,pw); toast('Sincronización configurada · recargando'); setTimeout(()=>location.reload(),800);}catch(e){toast('La config de Firebase no es válida (JSON)');} }
