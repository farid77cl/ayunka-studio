/* Ayünka Studio — sincronización en la nube (Firebase) con inicio anónimo.
...
(function(){
  ...
  function cfg(){
    try{
      const local=JSON.parse(localStorage.getItem(CKEY));
      if(local&&local.firebase) return local;
    }catch(e){}
    if(localStorage.getItem('ayunka-sync-disabled')==='true') return null;
    if(window.AYUNKA_CONFIG && window.AYUNKA_CONFIG.firebase) {
      return {
        firebase: window.AYUNKA_CONFIG.firebase,
        workspace: window.AYUNKA_CONFIG.workspace || 'ayunka'
      };
    }
    return null;
  }
  function configured(){ const c=cfg(); return !!(c&&c.firebase&&c.firebase.projectId); }
  function setCfg(firebaseObj,workspace){
    localStorage.removeItem('ayunka-sync-disabled');
    localStorage.setItem(CKEY,JSON.stringify({firebase:firebaseObj,workspace:workspace||'ayunka'}));
  }
  function clearCfg(){
    localStorage.removeItem(CKEY);
    localStorage.setItem('ayunka-sync-disabled','true');
  }
  ...
  async function push(){
    ...
    try{ window.DB._updatedAt=Date.now(); await st._fs.setDoc(st.ref,{db:window.DB,updatedAt:window.DB._updatedAt,owner:'anonymous'}); note('guardada en la nube '+new Date().toLocaleTimeString('es-CL')); }
  ...
  async function init(){
    ...
    try{
      ...
      note('iniciando sesión anónima…');
      try{ await authMod.signInAnonymously(auth); }
      catch(e){ note('error de login anónimo: '+(e.code||e.message)); return; }
