...
(function(){
  ...
  function renderProductModal(){
    ...
    const files=(p.files||[]).map(f=>`<div class="row between" style="margin:3px 0"><span>📄 ${esc(f.name)}</span><span class="row"><button class="btn ghost sm" onclick="A.prodOpenFile('${f.id}')">Abrir</button><button class="linkish" onclick="A.prodDelFile('${f.id}')">quitar</button></span></div>`).join('');
  ...
  function renderDayModal(){
    ...
    const jobs=d.jobs.map((j,i)=>{
      ...
      const files=pr&&pr.files&&pr.files.length?pr.files.map(f=>`<button class="btn ghost sm" onclick="A.dayOpenFile('${f.id}')">📄 Abrir ${esc(f.name)}</button>`).join(' '):'';
  ...
  function vAjustes(){const p=DB.params,b=p.business;const f=(id,lbl,val,step)=>`<label class="field">${lbl}<input id="${id}" type="number" ${step?`step="${step}"`:''} value="${val}"></label>`;
    return `<h1 class="page">Ajustes</h1><p class="sub">Parámetros de costeo y datos del negocio</p>
      ...
      <div class="card" style="margin-top:14px"><div class="sectiontitle" style="margin-top:0">Sincronización en la nube (celular ⇄ PC)</div>
        ...
        <div class="muted" style="margin-top:8px">Crea un proyecto gratis en Firebase, activa Firestore y Autenticación anónima, y pega aquí el firebaseConfig. La app debe estar publicada (GitHub Pages) para sincronizar. Las fotos/STL no se sincronizan por ahora.</div></div>`;}
  function syncSave(){ try{ const fb=JSON.parse(document.getElementById('sync-cfg').value); const ws=document.getElementById('sync-ws').value.trim()||'ayunka'; window.Sync.setCfg(fb,ws); toast('Sincronización configurada · recargando'); setTimeout(()=>location.reload(),700);}catch(e){toast('La config de Firebase no es válida (JSON)');} }
